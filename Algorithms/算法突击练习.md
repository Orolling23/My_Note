    ...
    public ListNode deleteDuplicates(ListNode head) {
        ...
        while (ptr.next != null && ptr.next.next != null) {
            if (ptr.next.val == ptr.next.next.val) {
                ...
                while (ptr2 != null && ptr.next.val == ptr2.val) {
                    ptr2 = ptr2.next;
                }
                ptr.next = ptr2;
            } else {
    ...
    public int longestValidParentheses(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (s.charAt(i) == '(') {
            	...
            	// 右括号就先弹栈（栈底至少有一个右括号），求当前右括号结尾的最长连续子串
                stack.pop();
                // 如果弹完栈为空，说明刚才弹的是一个右括号，那么当前右括号也没得匹配，所以直接压进去
                if (stack.isEmpty()) {
                    stack.push(i);
                } else {
                	// 否则，最大长度是当前右括号，到上一个无法被匹配的右括号
        ...
        for (int i = 1; i < s.length(); i++) {
            ...
            if (s.charAt(i) == ')') {
                ...
                if (s.charAt(i - 1) == '(') {
                	...
                	// 如果是 ... )) 形式,那么要检查看前面是否有对应的(
                    if (i - dp[i - 1] - 1 >= 0 && s.charAt(i - dp[i - 1] - 1) == '(') {
                        // 如果前面有对应的(，则当前长度是 = dp[i - 1]中间长度 + 2 + 左边的(的左侧的有效字串长度三部分
...
public class Solution {
    public boolean hasCycle(ListNode head) {
        // 一个快指针，一个慢指针，如果两个指针出发后又碰到一起了，说明有环
        ListNode ptr1 = head;
        ListNode ptr2 = head;
...
### 8. **输入n，给出1~n的所有排列情况（非全排列，1~n的数字可以不全使用）**
...
public class no8 {
    /*
    输入n，给出1~n的所有排列情况（非全排列，1~n的数字可以不全使用）
     */
    ...
    private static List<String> solve(int n) {
        ...
        // 目标长度从1 到 n
        for (int i = 1; i <= n; i++) {
            dfs(new ArrayList<Integer>(), i, n, new boolean[n + 1]);
        }
        return res;
    }
    ...
    /**
     *
     * @param list 记录过程字符的数组
     * @param len 目标长度
     * @param n 可用的数字总长度
     * @param used 记录哪些数字已经被用过
     */
    private static void dfs(ArrayList<Integer> list, int len, int n, boolean[] used) {
...
public class no14 {
    ...
    private static List<Integer> postOrder(TreeNode node, int k) {
        if (node == null) {
        ...
        List<Integer> left = postOrder(node.left, k);
        List<Integer> right = postOrder(node.right, k);
...
//     len 是 0~9，一共有10个
    private static int solve(int len, int n) {
        // dp[i][j] = k 表示 走第i步，到index为j的节点的方案数量为k
...
public class no19 {
    ...
    public static String mySqrt(int x) {
        if (x == 0) {
            return "0";
        }
        long xx = x * 10000;
...
//        System.out.println(left + " " + right);
        ...
        String res = ans + "";
        res = res.substring(0, res.length() - 2) + "." + res.substring(res.length() - 2);
        // int target = x - ans * ans;
        return res;
    }
}
```
...
class Solution {
    public int nextGreaterElement(int n) {
        ...
        int index = -1;
        for (int i = ch.length - 1; i >= 0 && index == -1; i--) {
            if (i - 1 >= 0 && ch[i - 1] < ch[i]) {
                for (int j = ch.length - 1; j >= i; j--) {
                    if (ch[j] > ch[i - 1]) {
                        // System.out.println((i - 1) + " " + j + " " + Arrays.toString(ch));
                        swap(ch, i - 1, j);
                        index = i;
                        // System.out.println((i - 1) + " " + j + " " + Arrays.toString(ch));
                        break;
        ...
        Arrays.sort(ch, index, ch.length);
