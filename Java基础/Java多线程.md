...
### volatile
volatile的实现方式就是在读写操作前后加屏障，具体到X86平台，就是在写操作后面加一个StoreLoad屏障，其他屏障都可以省略。这个屏障的实现方式是加一个`lock addl &0x0,(%esp)`lock的空操作（CPU缓存锁定），利用了**lock的两个特点：lock后面的指令不能重排序到lock前面、lock会将本线程的工作内存写入主内存，并引起其他线程无效化该值对应的工作内存。**  
...
我们可以看到这里有一个监视器的概念，就是加锁要有一个**被加锁的目标，目标相同的同步块才有竞争关系**。  
...
>
> * 偏向锁认为大多数时候都是一个线程获得锁
> * 轻量级锁认为大多数时候少数线程交替获得锁，不会竞争，或者轻量竞争自旋一下就行了
> * 重量级锁认为任何时候都可能会有竞争，悲观。
...
* addWaiter方法主要通过**CAS添加尾结点**，**这也是不用LinkedList而自己实现链表的原因之一**
...
* 执行完成后调用release方法释放锁，并且调用unpark方法使用**LockSupport唤醒等待状态的线程**
##### 共享式同步状态获取与释放
