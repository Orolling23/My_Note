...
* 数组扩容，当数组满时会重新定义一个原长度1.5倍长度的新数组，并将源数据复制到新数组中。 
...
### HashMap
* 数组 + 链表 + 红黑树组合而成，数组中保存Key-Value实例Node。  
* 扩容负载因子0.75f，即总容量100，存第76个的时候扩容。
* 扩容分为两步：1.创建一个新数组，长度是原来的2倍 2.Rehash。Rehash的原因是哈希随长度变化 index = HashCode（Key） & （Length - 1）
* 哈希冲突用拉链法解决，尾插法。JDK8之前是头插法，resize时会改变链表的顺序，多线程put时可能会产生环形链表。尾插法在多线程下不会产生环形链表，但可能会产生数据覆盖。  
