...
* 运行时常量池 —— 方法区的一部分，**存放编译器生成的各种字面量和符号引用**，线程共享
...
（注意：Java8 变成了程序计数器，虚拟机栈，堆，本地栈，元空间，运行时常量池元空间的一部分，直接内存，codecache）
...
**方法区中主要存储虚拟机加载的类型信息，静态变量，常量，即时编译器编译后的代码缓存等**。（1.8后String常量池到堆中去了，codecache单独一块了，方法区改名元空间，挪到本地内存空间去了）  
...
5. 由Eden区、From Space区向To Space区复制时，**对象大小大于To Space可用内存**，则把该对象通过分配担保转存到老年代，且老年代的可用内存小于该对象大小
